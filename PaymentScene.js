...
export class PaymentScene extends Phaser.Scene {
    ...
    _tryComplete() {
        ...
        // スコア加算
        // 🆕 正しい予約(+10)はボーナス扱い(isGlobal=true)、間違い(予約忘れ40,予約不要10)はミス扱い(isGlobal=false)
        if (reservationScore !== 0) {
            // 🚨 修正: 予約正解（予約対象者に予約作成）だけがボーナス、それ以外はペナルティ
            const isBonus = (reservationScore > 0 && reservationReason === '次回予約: 予約作成');
        ...
        if (noMistakes) {
            ...
            if (this.registry.get('isEndlessMode')) {
                const timeExt = gameState.getTimeExtension('payment');
                EventBus.emit(GameEvents.TIME_EXTENDED, {
                    seconds: timeExt,
                    reason: '会計完了'
                });
                console.log(`♾️ エンドレスモード: 会計完了 → +${timeExt}秒`);
            }
            
            // 🆕 パーティクルバースト演出
            EventBus.emit(GameEvents.PARTICLE_BURST, {
                x: 1680,  // 会計完了ボタン付近
                y: 880,
                color: 0x4CAF50,
                count: 20
