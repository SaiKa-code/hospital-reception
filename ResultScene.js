...
export class ResultScene extends Phaser.Scene {
    ...
    _restartGame() {
        ...
        // 4. GameStateManagerをリセット（スコア、コンボ、タイミングをリセット）
        GameStateManager.getInstance(this.game).resetGame();
        
        // 5. TutorialManagerをリセット
    ...
    _createRankEffects(rank) {
        ...
        switch (rank) {
            ...
            case 'F':
                // Fランク: 絶望的な音
                if (this.cache.audio.exists('se_score_d')) { // Fランクも既存のD用SEを流用または変更
    ...
    _calculateRank(score) {
        ...
        if (score > -100) return { rank: 'D', color: '#00d4aa', comment: '...満足に指導できなくてごめんなさい！！\n試用期間終了とさせていただきます' };
        // Fランク: -100点以下
        return { rank: 'F', color: '#FF0000', comment: '...もう二度と来ないでください！！\n警備員を呼びます！！' };
    }
