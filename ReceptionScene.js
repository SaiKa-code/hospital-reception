...
export class ReceptionScene extends Phaser.Scene {
    ...
    create() {
        ...
        // 🔥 高難易度モード判定
        this.isHardMode = this.registry.get('hardMode') || false;
        console.log(`[ReceptionScene] ハードモード: ${this.isHardMode}`);
        
        // 🔥 高難易度モード: 待ちゲージ更新タイマー
        if (this.isHardMode) {
            this.waitingGaugeTimer = this.time.addEvent({
                delay: 100, // 100msごとに更新
                callback: this._updateWaitingGauges,
                callbackScope: this,
                loop: true
            });
        }
        ...
        // 🚨 修正: sleepイベントでシーン全体の入力を無効化（ゴーストクリック完全防止）
        this.events.on('sleep', () => {
            console.log('[ReceptionScene] sleep - シーン全体の入力を無効化');
        ...
        this.events.on('wake', () => { 
            ...
            // 🚨 修正: wakeイベントでシーン全体の入力を再有効化
            console.log('[ReceptionScene] wake - シーン全体の入力を有効化');
        ...
        // 🆕 患者タイマーイベントの登録
        EventBus.on(GameEvents.PATIENT_TIMER_PAUSE, () => this._pausePatientTimers());
        EventBus.on(GameEvents.PATIENT_TIMER_RESUME, () => this._resumePatientTimers());
        ...
        this.time.delayedCall(500, () => {
            ...
            if (this.registry.get('startPracticeTutorial')) {
                ...
                return; // 🆕 start()内で表示が行われるため、重複を防ぐためにここで終了
            }
    ...
    _createPatient(data, x, y) {
        ...
        const maxImageNum = (gender === 'man') ? 14 : 15;
        ...
        // 🔥 高難易度モード: 待ちゲージ追加（全患者に検尿判断のゲージを表示）
        if (this.isHardMode) {
            const gaugeWidth = 100;
            const gaugeHeight = 8;
            const gaugeY = y + 310; // 名前タグと被らないよう少し下に配置
            
            // ゲージContainer（患者位置 x, gaugeY を基準に配置）
            const gaugeContainer = this.add.container(x, gaugeY);
            
            // ゲージ背景
            const gaugeBg = this.add.graphics();
            gaugeBg.fillStyle(0x333333, 0.8);
            gaugeBg.fillRoundedRect(-gaugeWidth/2, 0, gaugeWidth, gaugeHeight, 4);
            
            // ゲージ塗りつぶし
            const gaugeFill = this.add.graphics();
            gaugeFill.fillStyle(0x00ff00, 1);
            gaugeFill.fillRoundedRect(-gaugeWidth/2, 0, gaugeWidth, gaugeHeight, 4);
            
            // ⌛ 砂時計マーク（ゲージの左側に配置、丸い背景付き）
            const iconBg = this.add.graphics();
            iconBg.fillStyle(0x333333, 0.9);
            iconBg.fillCircle(-65, 4, 12);
            
            const hourglassIcon = this.add.text(-65, 4, '⌛', {
                fontSize: '14px'
            }).setOrigin(0.5);
            
            gaugeContainer.add([gaugeBg, gaugeFill, iconBg, hourglassIcon]);
            this.patientContainer.add(gaugeContainer);
    ...
    _pausePatientTimers() {
        ...
        if (this.isPatientTimerPaused) return;
        
        console.log('[ReceptionScene] 患者タイマー一時停止');
        this.isPatientTimerPaused = true;
        this.patientTimerPauseStartTime = Date.now();
        
        // タイマーイベント自体を停止
        if (this.waitingGaugeTimer) {
            this.waitingGaugeTimer.paused = true;
        }
    }
    
    _resumePatientTimers() {
        if (!this.isHardMode) return;
        if (!this.isPatientTimerPaused) return;
        
        const now = Date.now();
        const pauseDuration = now - (this.patientTimerPauseStartTime || now);
        console.log(`[ReceptionScene] 患者タイマー再開 (停止時間: ${pauseDuration}ms)`);
        
        // 全患者のゲージ開始時間をずらすことで、停止中の経過を無効化
        // elapsed = Date.now() - gauge.startTime なので
        // startTimeを pauseDuration だけ未来にずらせば elapsed は増えない
        this.patientManager?.patientQueue.forEach(p => {
            if (p.waitingGauge) {
                p.waitingGauge.startTime += pauseDuration;
            }
        });
        
        this.isPatientTimerPaused = false;
        this.patientTimerPauseStartTime = null;
        
        if (this.waitingGaugeTimer) {
            this.waitingGaugeTimer.paused = false;
        }
    }
    ...
    _showFixedPanel(data) {
        ...
        // 🆕 チュートリアル用ターゲットゾーン (Graphicsはwidth/heightを持たないため)
        const guideZone = this.add.zone(leftAreaX + guideWidth/2, guideY + 40, guideWidth, 80).setOrigin(0.5);
        this.activePatientUI.push(guideZone);
        TutorialManager.getInstance(this.game).registerButton('reception_guide_panel', guideZone);
        ...
        // 🆕 チュートリアル用ターゲットゾーン
        const g1Zone = this.add.zone(g1CenterX, pillY, pill1Width, pillHeight).setOrigin(0.5);
        this.activePatientUI.push(g1Zone);
        TutorialManager.getInstance(this.game).registerButton('reception_guide_shakai', g1Zone);
        ...
        // 🆕 チュートリアル用ターゲットゾーン
        const g2Zone = this.add.zone(g2CenterX, pillY, pill2Width, pillHeight).setOrigin(0.5);
        this.activePatientUI.push(g2Zone);
        TutorialManager.getInstance(this.game).registerButton('reception_guide_kokuho', g2Zone);
        ...
        // 🆕 チュートリアル用ターゲットゾーン
        const g3Zone = this.add.zone(g3CenterX, pillY, pill3Width, pillHeight).setOrigin(0.5);
        this.activePatientUI.push(g3Zone);
        TutorialManager.getInstance(this.game).registerButton('reception_guide_elderly', g3Zone);
                ...
                btnBg.on('pointerdown', () => {
                    ...
                    if (data.visuals && data.visuals.image) {
                        ...
                        if (data.questionnaireTimer) {
                            ...
                            // 🔥 高難易度モード: 問診をお願いしたフラグ
                            data.questionnaireAsked = true;
                            
                            // 🆕 修正: 問診開始時に待機ゲージを即座に消去
                            if (data.waitingGauge) {
                                if (data.waitingGauge.container) data.waitingGauge.container.destroy();
                                data.waitingGauge = null;
                            }
                    ...
                    this.time.delayedCall(20000, () => {
                        ...
                        // (旧: 問診完了時に待機ゲージを消去処理があった場所 - 削除済み)
                        
                        // 問診票完了ではコンボを加算しない（プレイヤーの操作ではないため）
                        ...
                        // 🆕 現在表示中の患者ならメインパネルも更新（主訴の反映）
                        if (this.currentPatient === data) {
                            this._updateActivePatientPanel(data);
                        }
    ...
    _showMyNumberFlow(data, x, y) {
        ...
        // 🆕 チュートリアル: マイナンバー確認ボタンを登録
        TutorialManager.getInstance(this.game).registerButton('mynumber_confirm_button', btn);
    }
    
    _showTypingFlow(data, x, y) {
        // 🆕 既にタイピングが完了している場合は、完了ボタンフローへスキップ
        if (data.isTypingCompleted) {
            this._showCompleteButton(data, x, y);
            return;
        }
    ...
    _launchSimpleTyping(data, type, displayTextObj, parentContainer) {
        ...
        htmlInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === 'Escape') {
                e.stopPropagation(); // 🆕 イベントのバブリングを停止（チュートリアル進行を防ぐ）
                e.preventDefault();
                confirmInput();
            }
        });
    ...
    _finishPatientProcess(data) {
        ...
        queue.forEach((p, i) => {
            ...
            // 🆕 修正: 待機ゲージと問診プログレスバーも一緒にスライドさせる
            if (p.waitingGauge && p.waitingGauge.container) {
                this.tweens.add({ targets: p.waitingGauge.container, x: targetX, duration: 500, ease: 'Power2' });
            }
            if (p.questionnaireProgressBar) {
                this.tweens.add({ targets: p.questionnaireProgressBar, x: targetX, duration: 500, ease: 'Power2' });
            }
        });
        ...
        selectedIndices.forEach((dataIndex, i) => {
            ...
            const patient = {
                ...
                imageKey: (() => {
                    ...
                    const maxNum = genderKey === 'man' ? 14 : 15;
    ...
    // 🔥 高難易度モード: 待ちゲージ更新処理
    // ==========================================================
    _updateWaitingGauges() {
        if (!this.isHardMode) return;
        
        const patients = this.patientManager?.patientQueue || [];
        patients.forEach(data => {
            if (!data.waitingGauge || data.isFinished) return;
            
            const gauge = data.waitingGauge;
            
            // 🆕 経過時間の計算 (pauseDuration考慮済み)
            const now = Date.now();
            const elapsed = now - gauge.startTime;
            const remaining = Math.max(0, gauge.timeout - elapsed);
            const ratio = remaining / gauge.timeout;
            
            // console.log(`[GaugeDebug] ${data.name}: elapsed=${elapsed}, remaining=${remaining}, type=${gauge.type}`);

            // ゲージ完了判定: 検尿と問診の両方が完了（または不要）であること
            // 🚨 修正: 検尿不要な場合でも、必ず判定（カップ有無の選択）を行うまでゲージを残す
            const urineFinished = data.playerGaveCup !== undefined;
            const questFinished = !data.needsQuestionnaire || data.questionnaireAsked;

            // 完了したらゲージ消去
            if (urineFinished && questFinished) {
                this._clearWaitingGauge(data);
                return;
            }
            
            // タイムアウト
            if (remaining <= 0) {
                // 優先度: 検尿未完了ならurine、そうでなければquestionnaire
                const reasonType = (!urineFinished) ? 'urine' : 'questionnaire';
                this._handlePatientTimeout(data, reasonType);
                return;
            }
            ...
            if (gauge.fill) {
                ...
                gauge.fill.fillStyle(color, 1);
                // Container内なので (0,0) 基準だが、RoundedRectは左上指定なので -width/2
                gauge.fill.fillRoundedRect(-gauge.gaugeWidth/2, 0, gauge.gaugeWidth * ratio, gauge.gaugeHeight, 4);
            }
        });
    }
    ...
    // 🔥 高難易度モード: 待ちゲージをクリア
    // ==========================================================
    _clearWaitingGauge(data) {
        if (!data.waitingGauge) return;
        
        // Containerごと削除
        if (data.waitingGauge.container) {
            data.waitingGauge.container.destroy();
        } else {
            // 後方互換性（念のため）
            const gauge = data.waitingGauge;
            if (gauge.bg) gauge.bg.destroy();
            if (gauge.fill) gauge.fill.destroy();
            if (gauge.label) gauge.label.destroy();
            if (gauge.iconBg) gauge.iconBg.destroy();
            if (gauge.hourglassIcon) gauge.hourglassIcon.destroy();
        }
        data.waitingGauge = null;
    }

    // ==========================================================
    // 🔥 高難易度モード: タイムアウト処理（患者帰宅）
    // ==========================================================
    _handlePatientTimeout(data, type) {
        console.log(`[ReceptionScene] 🚨 タイムアウト: ${data.name} (${type})`);
        ...
        // ペナルティスコア（数字で表示するためisMistake=false）
        const penaltyReason = type === 'urine' ? '検尿タイムアウト: 患者帰宅' : '問診タイムアウト: 患者帰宅';
        const hudScene = this.scene.get('HUDScene');
        if (hudScene && hudScene.addScore) {
            hudScene.addScore(-100, penaltyReason, true, false);
        }
        
        // 帰宅演出
        const img = data.visuals?.image;
        const nameTag = data.nameTag;
        
        if (img) {
            // 怒りマーク表示
            const angryMark = this.add.text(img.x, img.y - 100, '💢', { fontSize: '48px' }).setOrigin(0.5);
            this.patientContainer.add(angryMark);
            
            // SE再生
            if (this.sound.get('se_miss')) {
                this.sound.play('se_miss', { volume: 0.8 });
            }
            
            // フェードアウトして消える
            this.tweens.add({
                targets: [img, nameTag, angryMark, data.bellIcon].filter(Boolean),
                alpha: 0,
                x: img.x - 200,
                duration: 500,
                ease: 'Power2',
                onComplete: () => {
                    // 患者を完全に削除
                    this._removePatientFromQueue(data);
                }
            });
        } else {
            // ビジュアルがない場合は直接削除
    ...
    // 🔥 高難易度モード: 患者をキューから削除
    // ==========================================================
    _removePatientFromQueue(data) {
        const index = this.patientManager.patientQueue.indexOf(data);
        if (index > -1) {
            this.patientManager.patientQueue.splice(index, 1);
            console.log(`[ReceptionScene] 患者 ${data.name} を削除`);
        }
